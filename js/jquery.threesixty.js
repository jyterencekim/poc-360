...
; (function ($, window, document, undefined) {
    ...
    ThreeSixty.prototype.initLoad = function () {
        ...
        for (i; i < len; i++) {
            for (j = 1; j <= data[i].count; j++) {
                console.log(`Preloading ${pad(j)}`);
                url = data[i].path.replace('{index}', pad(j));
    ...
    ThreeSixty.prototype.onLoadAllComplete = function (objIndex) {
        ...
        for (i; i < l; i++) {
            ...
            html += '<img class="threesixty-frame" style="display:' + display + ';" data-index="' + i + '" src="' + pathTemplate.replace('{index}', pad(i + 1)) + '"/>';
        }
        $this.html(html);
